...
class LearningPinchTask(ShowBase, GripStateMachine):
    DATA_DIR = 'data'
    ...
    def __init__(self, id, session, hand, block, mode, wrist):
        ...
        self.prev_blk = os.path.join(self.DATA_DIR,'exp_4',self.sub_id,self.sess_id,self.wrist,self.hand,"B0")
        if self.wrist == 'pron':
            self.table = np.loadtxt('src/learning_pinch_task/trialtable_pron.csv',dtype='str',delimiter=',',skiprows=1)
        elif self.wrist == 'flex':
            self.table = np.loadtxt('src/learning_pinch_task/2trialtable_flex.csv',dtype='str',delimiter=',',skiprows=1)
        else:
        ...
        self.session_dir = os.path.join(self.DATA_DIR,'exp_4',self.sub_id,self.sess_id,self.wrist,self.hand)
        ...
        self.learn_rotmat = LEARNING_ROT_MAT(self.hand)
        ...
        self.grip_dir = os.path.join(self.DATA_DIR,'exp_4',self.sub_id,self.sess_id,self.wrist,self.hand,"B"+self.block)
    ...
    def read_data(self,task):
        ...
        if data is not None:
            ...
            data = np.dot(data,self.learn_rotmat)
            self.data = data
    ...
    def log_data(self, task):
        if (self.trial_counter + 1) <= self.table.shape[0]:
            if self.space:
                ...
                self.movvars = np.column_stack((self.ts, self.statenum, self.reportdata))
                self.statenum = []
                if self.mode=='task':
    ...
    def update_trial_command(self):
        ...
        if self.hand == 'Left':
            ...
            xfac = 0.25
        self.imageObject = OnscreenImage(image = str(self.table[self.trial_counter,3]),scale=(xfac,0.25,0.25),pos=(-1.5, 0, 0.3))
